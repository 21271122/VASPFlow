...
当前唯一允许安装的 DSH 版本为 `0.1.0-rc.6`。安装器会先执行 DSH 版本检查，检测到其他版本即停止，不会留下半安装状态。它优先使用 PATH 中的 `dsh`；Windows 上若 DSH 由 npx 缓存启动，也会自动定位缓存中的该版本。本插件尚未适配 DSH `0.1.2-rc.1` 的新版客户端模块体系。
